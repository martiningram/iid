...
% Package for notes
\usepackage[draft, inline=true, margin=false]{fixme}
...
tennis players between 2011 and 2015, we found that top players \fxnote{Though
  they are top players, maybe just go for players here? ``top'' sounds a bit
  like the top to me...?} were generally affected by the state of the score and
...
reveal about the mental game. \fxnote{If you'd like to add another idea, we
  could also say that tennis could be particularly suited for measuring mental
  effects, given that it's an individual sport and we have quite fine-grained
  data about the exact stage of the match? Not sure, just an idea -- but it's
  what Klaassen \& Magnus often mention, I think.}
...
that represent how players are affected by conditions \fxnote{I know you were
  looking for a different word. Later on, there's ``match state''; that sounds
  good to me. Or maybe ``match situation''?}  on average. The parameters
...
the opponent adjustment proposed by Barnett and Clarke \fxnote{maybe add
  \cite{barnett2005combining}?}. Klaassen and Magnus also proposed a dynamic
model to test for deviations from the IID model \fxnote{maybe add
  \cite{klaassen2001points}?}, which says that points in a match are independent
...
game, and set conditions \fxnote{...and represent our attempt to identify as
  many types of pressuring situations as possible...? Also, is it maybe worth
  mentioning what Klaassen \& Magnus chose and how and why we chose different
  ones?}. The majority of the point conditions focus on various types of
...
\fxnote{This is now called ``Just won'' I think?}), which captures short-term
...
\fxnote{I think this part describes the predictors well but it might be worth
  adding a little more of an explanation of why we chose them. Maybe add that
  they should capture a lot of different effects at different levels:
  match-level through importance, set up \& down, set level with the point
  spread, game level with missed opportunity and so on, and point level with
  break point etc.}
...
Klaassen and Magnus have previously shown \fxnote{maybe cite
  \cite{klaassen2001points}} that the linear model works well in practice for
...
was 6 for both tours. \fxnote{Was it different before?}  The average point
...
in this group. \fxnote{I like that you mention the players, but maybe ``most
  exciting'' could be a bit subjective (particularly for the hordes of Robredo
  fans)? Maybe tone down to ``considered to be most exciting'' or something?}
...
\fxnote{I do think the way the champions show up here is very cool, but do we
  completely understand what's going on, e.g. with Broady and the Pliskovas?
  Also, should we maybe mention that the outliers may be predictable in their
  own way which we are not capturing with the model, like you mentioned?  And
  perhaps offer some potential explanations -- different ``gears'', tactics?}
